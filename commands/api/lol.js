...
const res = await fetch(
    `http://ddragon.leagueoflegends.com/cdn/14.17.1/data/en_US/champion.json`
  ...
  const indexedChampion = await fetch(
    `http://ddragon.leagueoflegends.com/cdn/14.17.1/data/en_US/champion/${championNameID}.json`
  ).then((response) => response.json());
  ...
  if (randAbility === 4) {
    ...
    image = `http://ddragon.leagueoflegends.com/cdn/14.17.1/img/${abilityType}/${abilityID}`;
  } else {
    abilityID =
    ...
    image = `http://ddragon.leagueoflegends.com/cdn/14.17.1/img/${abilityType}/${abilityID}.png`;
  }
